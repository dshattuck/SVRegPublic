...
function thicknessISO(varargin)
% Usage:
...
nii = pvc;
nii.img = GMfrac;
save_untouch_nii_gz(nii, [subbase '.GM_frac_iso.nii.gz']);

%%% This is for LE method
%GMfrac = 1.0*(GMfrac>0.5);
...
save_untouch_nii_gz(nii, [subbase sprintf('.heat_map_sol_iso.nii.gz')], 64);
...
iso_thickness = interpn(volComp_x, volComp_y, volComp_z, thickness_vol, ...
   fv.vertices(:,1), fv.vertices(:,2), fv.vertices(:,3), 'linear');
fv.attributes = iso_thickness;
fv = colorDFS(fv, fv.attributes, [0 6], jet(256));
writedfs([subbase, sprintf('.iso-thickness_0-6mm.isosurface.dfs')], fv);
...
if isequal(size(inner.vertices), size(pial.vertices))
   if verLessThan('matlab', '7.14.1') % R2012a or lower
      F = TriScatteredInterp(fv.vertices(:,1), fv.vertices(:,2), fv.vertices(:,3), iso_thickness, 'nearest');
   else
      F = scatteredInterpolant(fv.vertices(:,1), fv.vertices(:,2), fv.vertices(:,3), iso_thickness, 'nearest', 'nearest');
   end
   mid = inner;
   ...
   writedfs([subbase, sprintf('.iso-thickness_0-6mm.mid.cortex.dfs')], mid);
end

%if exist([subbase, '.left.inner.cortex.svreg.dfs'],'file')
    split_thickness_map_iso(subbase);
%end

save([subbase, sprintf('.heat_sol_comp_iso.mat')], 'volComp_x', 'volComp_y', 'volComp_z', 'heat_map', ...
