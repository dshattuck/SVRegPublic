...
vnum=0;sur.faces=[];sur.vertices=[];sur.vcolor=[];sur.labels=[];
for kk=1:length(s)
    sur.vertices=[sur.vertices;s{kk}.vertices];
    if any(strcmp('labels',fieldnames(s{kk})))
        sur.labels=[sur.labels;s{kk}.labels];
    end
    if any(strcmp('vcolor',fieldnames(s{kk})))
        sur.vcolor=[sur.vcolor;s{kk}.vcolor];
    end
...
if isempty(sur.labels)
    sur=rmfield(sur,'labels');
end
if isempty(sur.vcolor)
    sur=rmfield(sur,'vcolor');
end
