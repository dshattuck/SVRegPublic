...
eps1=1e-4;
if nargin<1
   error('thicknessLE must have atleast one input.')
end
...
nii.img = GMfrac;
save_untouch_nii_gz(nii, [subbase '.GM_frac_LE.nii.gz']);
...
save_untouch_nii_gz(nii, [subbase sprintf('.heat_map_sol_LE.nii.gz')], 64);
...
writedfs([subbase, sprintf('.LE-thickness_0-6mm.isosurface.dfs')], fv);
...
if isequal(size(inner.vertices), size(pial.vertices))
   ...
   writedfs([subbase, sprintf('.LE-thickness_0-6mm.mid.cortex.dfs')], mid);
end

%if exist([subbase, '.left.inner.cortex.svreg.dfs'],'file')
    split_thickness_map_LE(subbase);
%end

save([subbase, sprintf('.heat_sol_comp_LE.mat')], 'volComp_x', 'volComp_y', 'volComp_z', 'heat_map', ...
