%  VIEW_NII_corr: Create or update a 3-View (Front, Top, Side) of the
%	brain data that is specified by nii structure
%
%  Usage:  	status = view_nii_bias_corr([h], nii, [option])	or
%		status = view_nii_bias_corr(h, [option])
%
...
%
function status = view_nii_bias_corr(varargin)

if nargin < 1
    error('Please check inputs using ''help view_nii_bias_corr''');
end;
...
switch command
    ...
    case {'updatenii'}
        ...
        view_nii_bias_corr(fig, opt);
        
        opt.command = 'init';
        view_nii_bias_corr(fig, nii, opt);
    ...
    case {'clearnii'}
        ...
        mymotion = '; view_nii_bias_corr(''move_cursor'');';
    ...
    case {'corrbias'}
        ...
        %v=nii_view.nii;
        nii_view = getappdata(fig,'nii_view');
        vals = get(nii_view.handles.Radius,'String');
        nii_view.radius=str2double(vals);

        vo=correct_bias_anand_chitresh(nii_view.nii,nii_view.gmvox,nii_view.wmvox,nii_view.csfvox, nii_view.radius);
        %view_nii(vo);
    case {'get3D'}
        nii_view = getappdata(fig,'nii_view');
        vals = get(h.c,'Value');
    ...
    case {'getgm'}
        nii_view = getappdata(fig,'nii_view');
        vals = get(nii_view.handles.Get3D,'Value');
        
        
        gm_vox=nii_view.gmvox;
        
        if ~isempty(nii_view.gmvox);
            if vals==0
                gm_vox=[gm_vox; nii_view.imgXYZ.vox];
            else
                for bx=-1:1
                    for by=-1:1
                        for bz=-1:1
                            gm_vox=[gm_vox;nii_view.imgXYZ.vox+[bx,by,bz]];
                        end
                    end
                end
            end
        ...
        nii_view.gmvox=gm_vox;
        
        % add point overlay
        bfc_idx = sub2ind(nii_view.dims, gm_vox(:,1),gm_vox(:,2), gm_vox(:,3));
        nii_view.nii.img(bfc_idx) = max(nii_view.nii.img(:));
        
        setappdata(fig, 'nii_view', nii_view);
        
        % update current figure
        view_nii_bias_corr('impos_edit');
    ...
    case {'getwm'}
        ...
        wm_vox=nii_view.wmvox;
        
        if ~isempty(nii_view.wmvox);
            if vals==0
                wm_vox=[wm_vox; nii_view.imgXYZ.vox];
            else
                for bx=-1:1
                    for by=-1:1
                        for bz=-1:1
                            wm_vox=[wm_vox;nii_view.imgXYZ.vox+[bx,by,bz]];
                        end
                    end
                end
            end
        ...
        nii_view.wmvox=wm_vox;
        
        % add point overlay
        bfc_idx = sub2ind(nii_view.dims, wm_vox(:,1),wm_vox(:,2), wm_vox(:,3));
        nii_view.nii.img(bfc_idx) = 0;%*max(nii_view.nii.img(:));
    ...
    case {'undogm'}
        ...
        if ~isempty(nii_view.gmvox);
            if vals==0
                if ~isempty(gm_vox)
                    gm_vox=gm_vox(1:end-1,:);
                end
            else
                if length(gm_vox)>=27
                    gm_vox=gm_vox(1:end-27,:);
                end
            end
        end
        nii_view.gmvox=gm_vox;
        nii_view.nii.img=nii_view.nii.orig;
        % add point overlay
        bfc_idx = sub2ind(nii_view.dims, nii_view.gmvox(:,1), nii_view.gmvox(:,2), nii_view.gmvox(:,3));
        nii_view.nii.img(bfc_idx) = max(nii_view.nii.img(:));
        bfc_idx = sub2ind(nii_view.dims, nii_view.wmvox(:,1), nii_view.wmvox(:,2), nii_view.wmvox(:,3));
        nii_view.nii.img(bfc_idx) = 0*max(nii_view.nii.img(:));
    ...
    case {'undowm'}
        ...
        if ~isempty(nii_view.wmvox);
            if vals==0
                if ~isempty(wm_vox)
                    wm_vox=wm_vox(1:end-1,:);
                end
            else
                if length(wm_vox)>=27                
                    wm_vox=wm_vox(1:end-27,:);
                end
            end
        end
        nii_view.wmvox=wm_vox;
        ...
        %    case {'getcsf'}
        %       nii_view = getappdata(fig,'nii_view');
        %       csf_vox=nii_view.csfvox;
        %       if ~isempty(nii_view.csfvox);
        %          csf_vox=[csf_vox;nii_view.imgXYZ.vox];
        %       else
        %          csf_vox=nii_view.imgXYZ.vox;
        %       end
        %       nii_view.csfvox=csf_vox;
        %       %set(nii_view.gmvox,'value',gm_vox);
        %       setappdata(fig, 'nii_view', nii_view);
...
nii_view.radius=1500;
nii_view.dims = dims;
nii_view.voxel_size = voxel_size;
nii_view.origin = origin;
nii_view.gmvox=zeros(0,3);nii_view.wmvox=zeros(0,3);nii_view.csfvox=zeros(0,3);
...
nii_view.fig = fig;
nii_view.nii = nii;			
%nii_view.nii.orig=nii_view.nii.img;% image data
...
if xdim > 1,
    ...
    handles.sagittal_slider = uicontrol('Parent',fig, ...
        ...
        'Callback','view_nii_bias_corr(''sagittal_slider'');');
...
if ydim > 1,
    ...
    handles.coronal_slider = uicontrol('Parent',fig, ...
        ...
        'Value',slider_val,...
        'Callback','view_nii_bias_corr(''coronal_slider'');');
...
if zdim > 1,
    ...
    handles.axial_slider = uicontrol('Parent',fig, ...
        ...
        'Callback','view_nii_bias_corr(''axial_slider'');');
...
handles.impos = uicontrol('Parent',fig,'Style','edit', ...
    ...
    'BusyAction','queue',...
    'Callback','view_nii_bias_corr(''impos_edit'');', ...
...
if 0
    ...
    handles.coord = uicontrol('Parent',fig,'Style','popupmenu', ...
        ...
        'visible','off', ...
        'Callback','view_nii_bias_corr(''coordinates'');');
...
handles.xhair_color = uicontrol('Parent',fig,'Style','push', ...
    ...
    'String','Color',...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''xhair_color'');');
...
handles.xhair = uicontrol('Parent',fig,'Style','popupmenu', ...
    ...
    'String',{'On','Off'},...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''crosshair'');');
...
handles.hist_eq = uicontrol('Parent',fig,'Style','toggle', ...
    ...
    'String','Hist EQ',...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''hist_eq'');');
...
handles.hist_plot = uicontrol('Parent',fig,'Style','push', ...
    ...
    'String','Hist Plot',...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''hist_plot'');');
...
handles.neg_color = uicontrol('Parent',fig,'Style','toggle', ...
    ...
    'String','Negative',...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''neg_color'');');
...
handles.colorindex = uicontrol('Parent',fig,'Style','popupmenu', ...
    ...
    'value', colorindex, ...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''color'');');
...
handles.contrast = uicontrol('Parent',fig, ...
    ...
    'Value',1, ...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''contrast'');');
...
if nii_view.numscan > 1
    set(handles.contrast, 'min', 1, 'max', nii_view.numscan, ...
        ...
        'Callback', 'view_nii_bias_corr(''slider_change_scan'');');
...
handles.brightness = uicontrol('Parent',fig, ...
    ...
    'Value',0, ...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''brightness'');');
...
handles.CorrBias = uicontrol('Parent',fig,'Style','push', ...
    'Units','Normalized', Font, ...
    'Position',pos, 'HorizontalAlignment', 'center',...
    'TooltipString','Restore initial contrast',...
    'String','Corr Bias',...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''CorrBias'');');
if usepanel
    set(handles.CorrBias, 'visible', 'on');
end
...
handles.GetGM = uicontrol('Parent',fig,'Style','push', ...
    'Units','Normalized', Font, ...
    'Position',pos, 'HorizontalAlignment', 'center',...
    'TooltipString','Restore initial contrast',...
    'String','GM',...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''GetGM'');');
if usepanel
    set(handles.GetGM, 'visible', 'on');
end
...
handles.Get3D = uicontrol('Parent',fig,'Style','checkbox', ...
    'Units','Normalized', Font, ...
    'Position',pos, 'HorizontalAlignment', 'center',...
    'TooltipString','Restore initial contrast',...
    'String','3D',...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''Get3D'');');
if usepanel
    set(handles.Get3D, 'visible', 'on');
end
...
handles.Radius = uicontrol('Parent',fig,'Style','edit', ...
    'Units','Normalized', Font, ...
    'Position',pos, 'HorizontalAlignment', 'center',...
    'TooltipString','Restore initial contrast',...
    'String','1500','Value',1500,...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''Radius'');');
if usepanel
    set(handles.Radius, 'visible', 'on');
end
...
handles.MedGM = uicontrol('Parent',fig,'Style','text', ...
    'Units','Normalized', Font, ...
    'Position',pos, 'HorizontalAlignment', 'center',...
    'TooltipString','Median Value of the Gray Matter',...
    'String',sprintf('GM Med:%4.3f',nii.gm_avg),...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''MedGM'');');
if usepanel
    set(handles.MedGM, 'visible', 'on');
end
...
handles.MedWM = uicontrol('Parent',fig,'Style','text', ...
    'Units','Normalized', Font, ...
    'Position',pos, 'HorizontalAlignment', 'center',...
    'TooltipString','Median Value of the Gray Matter',...
    'String',sprintf('WM Med:%4.3f',nii.wm_avg),...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''MedWM'');');
if usepanel
    set(handles.MedWM, 'visible', 'on');
end
...
handles.GetWM = uicontrol('Parent',fig,'Style','push', ...
    'Units','Normalized', Font, ...
    'Position',pos, 'HorizontalAlignment', 'center',...
    'TooltipString','Restore initial contrast',...
    'String','WM',...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''GetWM'');');
if usepanel
    set(handles.GetWM, 'visible', 'on');
end
...
handles.Undo1 = uicontrol('Parent',fig,'Style','push', ...
    'Units','Normalized', Font, ...
    'Position',pos, 'HorizontalAlignment', 'center',...
    'TooltipString','Restore initial contrast',...
    'visible','off', ...
    'String','UndoGM',...
    'Callback','view_nii_bias_corr(''UndoGM'');');

if usepanel
    set(handles.Undo1, 'visible', 'on');
end
...
handles.Undo2 = uicontrol('Parent',fig,'Style','push', ...
    'Units','Normalized', Font, ...
    'Position',pos, 'HorizontalAlignment', 'center',...
    'TooltipString','Restore initial contrast',...
    'visible','off', ...
    'String','UndoWM',...
    'Callback','view_nii_bias_corr(''UndoWM'');');

if usepanel
    set(handles.Undo2, 'visible', 'on');
end
...
% Font.FontSize   = 18;
%
% handles.GetCSF = uicontrol('Parent',fig,'Style','push', ...
%    'Units','Normalized', Font, ...
%    'Position',pos, 'HorizontalAlignment', 'center',...
%    'TooltipString','Restore initial contrast',...
%    'String','CSF',...
%    'visible','off', ...
%    'Callback','view_nii_bias_corr(''GetCSF'');');
% if usepanel
%    set(handles.GetCSF, 'visible', 'on');
% end
...
handles.contrast_def = uicontrol('Parent',fig,'Style','push', ...
    ...
    'String','Reset',...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''contrast_def'');');
...
if nii_view.numscan > 1
    set(handles.contrast_def, 'style', 'edit', 'background', 'w', ...
        ...
        'string', '1', 'Callback', 'view_nii_bias_corr(''edit_change_scan'');');
elseif colorindex < 2 | colorindex > 3
...
handles.brightness_def = uicontrol('Parent',fig,'Style','push', ...
    ...
    'String','Reset',...
    'visible','off', ...
    'Callback','view_nii_bias_corr(''brightness_def'');');
...
h1 = plot_view(fig, xdim, ydim, top_ax, img_slice, clim, cbarminmax, ...
    handles, useimagesc, colorindex, color_map, ...
    colorlevel, highcolor, useinterp, nii_view.numscan);
set(h1,'buttondown','view_nii_bias_corr(''axial_image'');');
...
h1 = plot_view(fig, xdim, zdim, front_ax, img_slice, clim, cbarminmax, ...
    handles, useimagesc, colorindex, color_map, ...
    colorlevel, highcolor, useinterp, nii_view.numscan);
set(h1,'buttondown','view_nii_bias_corr(''coronal_image'');');
...
h1 = plot_view(fig, ydim, zdim, side_ax, img_slice, clim, cbarminmax, ...
    handles, useimagesc, colorindex, color_map, ...
    colorlevel, highcolor, useinterp, nii_view.numscan);
set(h1,'buttondown','view_nii_bias_corr(''sagittal_image'');');
...
if ~isempty(nii_view.handles.axial_image)
    ...
    set(nii_view.handles.axial_image,'buttondown','view_nii_bias_corr(''axial_image'');');
end
...
if ~isempty(nii_view.handles.coronal_image)
    ...
    set(nii_view.handles.coronal_image,'buttondown','view_nii_bias_corr(''coronal_image'');');
end
...
if ~isempty(nii_view.handles.sagittal_image)
    ...
    set(nii_view.handles.sagittal_image,'buttondown','view_nii_bias_corr(''sagittal_image'');');
end
...
view_nii_bias_corr(fig, nii_view.nii.img, opt);
